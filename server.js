...
function requireClientKey(req, res, next) {
  ...
  if (PUBLIC_MODE || (CLIENT_KEYS.length === 0 && TIER_TOP.length === 0 && TIER_MID.length === 0 && TIER_LOW.length === 0)) {
  ...
  // Check Top Tier
  if (key === "admin-key" || key.startsWith("pro-user-") || TIER_TOP.includes(key)) {
    req.userTier = 'top';
    return next();
  }
...
// Generic pass-through for Football providers
app.get("/api/football/proxy", requireClientKey, async (req, res) => {
  try {
    ...
    if (FOOTBALL_PROVIDER === "apifootball") {
      ...
      const url = `${FOOTBALL_BASE_URL}${safePath}`;
      const data = await fetchJSON(url, { headers: { "x-apisports-key": key } });
      return res.json(data);
    }
    if (FOOTBALL_PROVIDER === "footballdata") {
      ...
      if (!token) return res.status(400).json({ error: "FOOTBALL_DATA_TOKEN missing in env." });
      const safePath = path.replace(/^\/?/, "/");
      const url = `https://api.football-data.org/v4${safePath}`;
      const data = await fetchJSON(url, { headers: { "X-Auth-Token": token } });
      return res.json(data);
    }
    // TheSportsDB generic pass-through
    const tsdbKey = process.env.THESPORTSDB_KEY || "3";
    const safePath = path.replace(/^\/?/, "/");
    const url = `https://www.thesportsdb.com/api/v1/json/${encodeURIComponent(tsdbKey)}${safePath}`;
    const data = await fetchJSON(url);
    return res.json(data);
  } catch (e) {
    res.status(500).json({ error: String(e?.message || e) });
...
// Generic pass-through for API-SPORTS sports
app.get("/api/sport/:sport/proxy", requireClientKey, async (req, res) => {
  try {
    ...
    const base = sportConfig[sport];
    if (!base) return res.status(404).json({ error: "Sport not supported by proxy" });
    const safePath = path.replace(/^\s*https?:\/\/[^/]+/i, "").replace(/^\/?/, "/");
...
app.get("/api/racing/upcoming", requireClientKey, async (_req, res) => {
  try {
    if (process.env.RACING_BASE_URL) {
      ...
      if (!path) {
        const regionRaw = (process.env.RACING_REGION_CODES || "gb").trim();
        const region = regionRaw.split(/[,\s]+/)[0].toLowerCase();
      ...
      try {
        ...
        // If provider rejects region_codes value, retry with 'gb' then without region_codes
        if (/unrecognis(?:ed|ed).+region.+code/i.test(msg)) {
          const u = new URL(url);
          u.searchParams.set("region_codes", "gb");
          try {
            const data3 = await fetchJSON(u.toString(), { headers });
            return res.json(data3);
          } catch (_) {
            u.searchParams.delete("region_codes");
            const data4 = await fetchJSON(u.toString(), { headers });
...
// Racing generic proxy to your configured provider base
app.get("/api/racing/proxy", requireClientKey, async (req, res) => {
  try {
    const base = process.env.RACING_BASE_URL;
    if (!base) return res.status(400).json({ error: "RACING_BASE_URL not set" });
...
// ======== Brain (OpenAI) ========
app.get("/api/brain/verify", async (_req, res) => {
  try {
    ...
    const msg = json?.error?.message || r.statusText || "Unknown";
    return res.status(200).json({ ok: false, status: r.status, reason: msg });
  } catch (e) {
...
app.post("/api/brain/analyse", requireClientKey, async (req, res) => {
  try {
    ...
    if (!OPENAI_API_KEY) {
      console.warn("OPENAI_API_KEY missing. Brain offline (no simulation).");
      return res.status(503).json({ error: "BRAIN_OFFLINE" });
    }
    ...
    if (!r.ok) {
      ...
      if (r.status === 401 || code === "invalid_api_key") {
        return res.status(503).json({ error: "BRAIN_INVALID_KEY", details: msg });
      }
...
// ======== Env Summary (masked) ========
app.get("/api/env/summary", (_req, res) => {
  try {
    ...
    const mask = (k, v) => {
      ...
      if (["key","token","secret","password","bearer"].some(w => key.includes(w))) {
    ...
    const summary = {
      ...
      PORT: env.PORT || "",
      FOOTBALL_PROVIDER: env.FOOTBALL_PROVIDER || "",
      FOOTBALL_BASE_URL: env.FOOTBALL_BASE_URL || "",
      FOOTBALL_API_KEY: mask("FOOTBALL_API_KEY", env.FOOTBALL_API_KEY || env.API_FOOTBALL_KEY),
      RACING_BASE_URL: env.RACING_BASE_URL || "",
      RACING_USERNAME: mask("RACING_USERNAME", env.RACING_USERNAME),
      RACING_PASSWORD: mask("RACING_PASSWORD", env.RACING_PASSWORD),
      RACING_BEARER_TOKEN: mask("RACING_BEARER_TOKEN", env.RACING_BEARER_TOKEN),
      RACING_REGION_CODES: env.RACING_REGION_CODES || "",
      RACING_LIMIT: env.RACING_LIMIT || "",
      OPENAI_MODEL: env.OPENAI_MODEL || "",
      OPENAI_API_KEY: mask("OPENAI_API_KEY", env.OPENAI_API_KEY),
      CLIENT_KEYS_count: (env.CLIENT_KEYS || "").split(",").filter(Boolean).length,
      TIER_TOP_KEYS_count: (env.TIER_TOP_KEYS || "").split(",").filter(Boolean).length,
      TIER_MID_KEYS_count: (env.TIER_MID_KEYS || "").split(",").filter(Boolean).length,
      TIER_LOW_KEYS_count: (env.TIER_LOW_KEYS || "").split(",").filter(Boolean).length
    };
